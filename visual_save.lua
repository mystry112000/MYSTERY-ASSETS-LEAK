...
if terrain then
    ...
    -- Iterate through the entire terrain to find painted cells
    local worldSize = terrain.Size
    local resolution = 4 -- check every 4 cells to keep it fast
    local paintedCells = {}
    
    -- Only scan a reasonable area to avoid lag
    ...
    if success and materialArray then
        -- Material colors are stored in terrain.MaterialColors
        -- This preserves the color palette used
...
saveinstance({
    SaveTerrain = true,          -- Save terrain material data
    StreamOnly = false,          -- Get everything, not just streamed parts
    Scripts = false,             -- Skip Lua scripts (set true if you want them)
    RemoveDefaultTags = true,    -- Clean up default tags
    RemoveCollision = false,     -- Keep collision data
    Compress = true              -- Compress the output file
})
